# XPlayer2 1.1.1b — your computer, in your glasses

**This is a pre-release.** It is the largest addition XPlayer2 has had, and it wants real use on
real hardware before it goes to everyone. If something is wrong, it is worth more to us in a bug
report than in silence.
...
crispness instead of the soft edges you get from a naive downscale. And it is no longer stretched
to fill the panel: a 16:10 desktop stays 16:10 in 16:9 glasses, letterboxed rather than squashed.

**Colours that are actually your colours.** The stream now carries its colour signalling instead of
...
- **Its own remote**, separate from the film remote and built like it — two live numbers that tell
  you whether the stream is healthy, the details behind a door for when it is not, and a screen that
  goes black in your lap so a lit phone does not shine up into the glasses.
- **Spatial audio for films** (off by default) on routes where the platform can render it.
- Dozens of fixes found by an adversarial review of the new code, before you ever saw it.
...
XPlayer2 for iPhone and iPad has all of this too, PC Link included. It will be **available on the
App Store when it is released** — same features, same desktop app, same pairing.
